...
exports.ReportsService = exports.ReportScopeService = void 0;
...
let ReportScopeService = class ReportScopeService {
    prisma;
    departmentScope;
    constructor(prisma, departmentScope) {
        this.prisma = prisma;
        this.departmentScope = departmentScope;
    }
    async scopedUserIds(actor, departmentId) {
        if (actor.roles.includes('ADMIN') || actor.permissions.includes('report.payroll.detail'))
            return undefined;
        if (departmentId)
            this.departmentScope.assertDepartmentAccess(actor, departmentId);
...
exports.ReportScopeService = ReportScopeService;
exports.ReportScopeService = ReportScopeService = __decorate([
...
let ReportsService = class ReportsService {
    ...
    async tasks(query, actor) {
        ...
        const [total, newly, accepted, inProgress, waitingReview, completed, overdue] = await Promise.all([
            ...
            this.prisma.taskAssignment.count({ where: { ...where, status: client_1.TaskAssignmentStatus.NEW } }),
            this.prisma.taskAssignment.count({ where: { ...where, status: client_1.TaskAssignmentStatus.ACCEPTED } }),
            this.prisma.taskAssignment.count({ where: { ...where, status: client_1.TaskAssignmentStatus.IN_PROGRESS } }),
            this.prisma.taskAssignment.count({ where: { ...where, status: client_1.TaskAssignmentStatus.WAITING_REVIEW } }),
            this.prisma.taskAssignment.count({ where: { ...where, status: client_1.TaskAssignmentStatus.COMPLETED } }),
            this.prisma.taskAssignment.count({ where: { ...where, assignmentDueAt: { lt: new Date() }, status: { notIn: [client_1.TaskAssignmentStatus.COMPLETED, client_1.TaskAssignmentStatus.CANCELLED] } } }),
        ]);
    ...
    async payroll(_query, actor) {
        if (!actor.permissions.includes('report.payroll.summary') && !actor.permissions.includes('report.payroll.detail')) {
            throw (0, error_util_1.forbidden)('PAYROLL_REPORT_FORBIDDEN', 'Cannot access payroll report');
        }
        const aggregate = await this.prisma.payroll.aggregate({
            where: { status: { in: [client_1.PayrollStatus.APPROVED, client_1.PayrollStatus.LOCKED] } },
            _count: { id: true },
    ...
    async assets() {
        const [total, assigned, inStock, maintenance, damaged, lost, disposed] = await Promise.all([
            ...
            this.prisma.asset.count({ where: { assetStatus: client_1.AssetStatus.ASSIGNED } }),
            this.prisma.asset.count({ where: { assetStatus: client_1.AssetStatus.IN_STOCK } }),
            this.prisma.asset.count({ where: { assetStatus: client_1.AssetStatus.MAINTENANCE } }),
            this.prisma.asset.count({ where: { assetStatus: client_1.AssetStatus.DAMAGED } }),
            this.prisma.asset.count({ where: { assetStatus: client_1.AssetStatus.LOST } }),
            this.prisma.asset.count({ where: { assetStatus: client_1.AssetStatus.DISPOSED } }),
        ]);
        return [{ total, assigned, inStock, maintenance, damaged, lost, disposed }];
    }
    async kpi(query, actor) {
        const userIds = await this.scope.scopedUserIds(actor, query.departmentId);
        ...
        const finalized = await this.prisma.employeeKpiAssignment.count({ where: { userId: userIds ? { in: userIds } : query.userId, kpiTemplateId: query.templateId, status: client_1.EmployeeKpiAssignmentStatus.FINALIZED } });

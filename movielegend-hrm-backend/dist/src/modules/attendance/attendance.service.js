...
let AttendanceService = class AttendanceService {
    ...
    async getDashboardStats(actor, query) {
        const visibleDepartmentIds = this.scope.visibleDepartmentIds(actor);
        const departmentFilter = this.departmentFilter(query.departmentId, visibleDepartmentIds);
        ...
        const totalUsersCount = await this.prisma.user.count({
            where: {
                ...
                ...(departmentFilter ? { departmentLinks: { some: { departmentId: departmentFilter } } } : {}),
            },
        });
        ...
        for (const r of records) {
            if (!r.checkInAt || !r.shiftAssignment?.shift)
                continue;
    ...
    async removeLocation(id) {
        ...
        if (!location)
            throw (0, error_util_1.notFound)('LOCATION_NOT_FOUND', 'Khong tim thay diem cham cong');
        return this.prisma.attendanceLocation.update({
            where: { id },
            data: {
                isActive: false,

...
let PermissionsGuard = class PermissionsGuard {
    ...
    canActivate(context) {
        ...
        const anyRequired = this.reflector.getAllAndOverride(any_permissions_decorator_1.ANY_PERMISSIONS_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        const allowed = (required?.every((permission) => permissions.has(permission)) ?? true) &&
            (!anyRequired?.length || anyRequired.some((permission) => permissions.has(permission)));
        if (!allowed) {
            throw (0, error_util_1.forbidden)('FORBIDDEN', 'Bạn không có quyền thực hiện thao tác này');
        }
        return true;
    }
};

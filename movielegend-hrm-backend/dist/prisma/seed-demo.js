...
async function main() {
    ...
    const departments = await Promise.all(departmentNames.map((name) => prisma.department.upsert({
    ...
    for (const [email, phone, fullName, roleCode, departmentId] of users) {
        ...
        const user = await prisma.user.upsert({
            where: { phone },
            update: { email, passwordHash, accountStatus: client_1.AccountStatus.ACTIVE, approvalStatus: client_1.ApprovalStatus.APPROVED, isActive: true },
            create: {
                userCode,
                phone,
                email,
                ...
                isActive: true,
                profile: { create: { fullName, idCardNumber: `DEMO-${phone}`, employmentStatus: client_1.EmploymentStatus.OFFICIAL, joinDate: new Date() } },
            },
        });
        const role = roleByCode.get(roleCode);
        if (role) {
            const scopeType = roleCode === 'LEADER' ? client_1.RoleScopeType.DEPARTMENT : client_1.RoleScopeType.GLOBAL;
            const scopeId = roleCode === 'LEADER' ? departmentId : null;
            const existingRole = await prisma.userRole.findFirst({
                where: { userId: user.id, roleId: role.id, scopeType, scopeId },
            });
            if (!existingRole)
                await prisma.userRole.create({ data: { userId: user.id, roleId: role.id, scopeType, scopeId } });
        }
    ...
    const period = await prisma.payrollPeriod.upsert({
        ...
        update: {},
        create: { periodCode: `DEMO-${workDate.getFullYear()}-${workDate.getMonth() + 1}`, companyId: company.id, month: workDate.getMonth() + 1, year: workDate.getFullYear(), startDate: workDate, endDate: workDate, createdById: employee.id, status: client_1.PayrollPeriodStatus.CALCULATED },
    });
    await prisma.payroll.upsert({
        where: { payrollPeriodId_userId: { payrollPeriodId: period.id, userId: employee.id } },
        update: {},
        create: { payrollPeriodId: period.id, userId: employee.id, salaryProfileId: salaryProfile.id, baseSalary: 10000000, standardWorkingDays: 26, calculatedAt: new Date(), status: client_1.PayrollStatus.CALCULATED },
    });
    const template = await prisma.contractTemplate.upsert({
        where: { companyId_code: { companyId: company.id, code: 'DEMO_CONTRACT' } },
        update: {},
        create: { companyId: company.id, code: 'DEMO_CONTRACT', name: 'Demo Contract', contractType: client_1.ContractType.FIXED_TERM, templateFileUrl: 'local://demo-contract.docx', createdById: employee.id },
    });
    ...
    await prisma.employeeContract.upsert({
        ...
        update: {},
        create: { contractCode: 'DEMO-CTR-001', userId: employee.id, contractTemplateId: template.id, contractTemplateVersionId: version.id, contractType: client_1.ContractType.FIXED_TERM, title: 'Demo Employee Contract', startDate: workDate, status: client_1.ContractStatus.ACTIVE, createdById: employee.id },
    });
    const kpiTemplate = await prisma.kpiTemplate.upsert({
        where: { companyId_code: { companyId: company.id, code: 'DEMO_KPI' } },
        update: {},
        create: { companyId: company.id, departmentId: departments[2].id, code: 'DEMO_KPI', name: 'Demo KPI', periodType: client_1.KpiPeriodType.MONTHLY, createdById: employee.id },
    });
    const criteria = await prisma.kpiCriteria.upsert({
        where: { kpiTemplateId_code: { kpiTemplateId: kpiTemplate.id, code: 'TASK_DONE' } },
        update: {},
        create: { kpiTemplateId: kpiTemplate.id, code: 'TASK_DONE', name: 'Task completion', weight: 100, scoringMethod: client_1.KpiScoringMethod.MANUAL },
    });
    const assignment = await prisma.employeeKpiAssignment.upsert({
        ...
        update: {},
        create: { userId: employee.id, kpiTemplateId: kpiTemplate.id, periodStart: workDate, periodEnd: workDate, status: client_1.EmployeeKpiAssignmentStatus.ACTIVE, assignedById: employee.id },
    });

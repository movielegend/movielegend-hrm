...
import { PrismaClient, TaskPriority, TaskStatus, TaskTargetType, TaskType, AttendanceStatus, ShiftAssignmentStatus } from '@prisma/client';
...
async function main() {
  ...
  // 1. Tìm Users (Admin, Leader, Employee)
  ...
  const empPermissions = await prisma.rolePermission.findMany({
    where: { roleId: { in: employeeData.roles.map((r: any) => r.roleId) } },
    include: { permission: true },
  });

  const leaderPermissions = await prisma.rolePermission.findMany({
    where: { roleId: { in: leaderData.roles.map((r: any) => r.roleId) } },
    include: { permission: true },
  });
  ...
  const empActor: AuthenticatedUser = {
    ...
    roles: employeeData.roles.map((r: any) => r.role.code),
    permissions: empPermissions.map(rp => rp.permission.code),
    scopes: employeeData.roles.map((r: any) => ({ role: r.role.code, scopeType: r.scopeType, scopeId: r.scopeId })),
  };

  const leaderActor: AuthenticatedUser = {
    sub: leaderData.id,
    userId: leaderData.id,
    roles: leaderData.roles.map((r: any) => r.role.code),
    permissions: leaderPermissions.map(rp => rp.permission.code),
    scopes: leaderData.roles.map((r: any) => ({ role: r.role.code, scopeType: r.scopeType, scopeId: r.scopeId })),
  };

  // 2. Test luồng Chấm công nâng cao (Check-in, Check-out, Yêu cầu điều chỉnh)
  console.log(`[2] ATTENDANCE FLOW: Check-in, Check-out, Adjustment`);
  
  // Tạo ShiftAssignment giả để checkin nếu chưa có hôm nay
  ...
  // Xóa attendance cũ hôm nay nếu có để test sạch
  ...
  try {
    ...
    const task = await tasksService.create(
      {
        ...
        targets: [
          { targetType: TaskTargetType.USER, targetId: empActor.userId }
        ]
      },
      leaderActor
    );
    ...
    // Employee Xin gia hạn (Extension)
    const newDueAt = new Date(dueAt);
    newDueAt.setDate(newDueAt.getDate() + 3);
    const extension = await tasksService.requestExtension(
      task.id,
      { assignmentId, requestedDueAt: newDueAt.toISOString(), reason: 'Cần thêm thời gian lấy số liệu kênh B' },
      empActor
    );
    console.log(`  -> EMPLOYEE XIN GIA HẠN THÀNH CÔNG`);

    // Leader duyệt gia hạn
    await tasksService.approveExtension(extension.id, leaderActor);
    console.log(`  -> LEADER DUYỆT GIA HẠN THÀNH CÔNG`);

    // Employee Submit (Nộp task)
  ...
  try {
    ...
    // 4.2 Test Contract
    // Need a contract template first

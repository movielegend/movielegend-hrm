...
export function mapTaskError(code: string, fallback: string): string {
  const map: Record<string, string> = {
    ...
    INVALID_EXTENSION_DUE_AT: 'Ngày gia hạn phải ở tương lai.',
    EXTENSION_LIMIT_EXCEEDED: 'Bạn đã đạt giới hạn tối đa 2 lần gia hạn cho công việc này.',
    TASK_EXTENSION_ALREADY_PROCESSED: 'Yêu cầu gia hạn đã được xử lý.',
